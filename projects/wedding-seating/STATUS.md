...
status: active build — MVP core + usability + attributes/colors + multi-group + venue catalog + walls/chairs done
last_updated: 2026-07-10
branch: feat/wedding-seating-engine (68+ commits ahead of master, NOT merged)
...
- **Plan 10 — PDF export** with color coding + legend + per-table list (the printable deliverable for couples/venues). NEXT logical step.
- **Positioned layout templates** by guest-count range (venue note 3) — deferred as the heaviest, least-essential piece.
...
## Verification status
- 104 automated tests green; `tsc --noEmit` + `next build` clean. Every plan's UI was driven end-to-end with Playwright (generate, drag, colors, locks, swap, catalog, spacing, boundary, chairs) with screenshots in `.superpowers/`.
